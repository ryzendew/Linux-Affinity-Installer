...
################################################################################
# Affinity v3 Installation Script for Linux
# This script automates the installation of Affinity applications on Linux
...
################################################################################
# SECTION 8: Affinity Application Installation
################################################################################

print_header "Affinity Application Installation"
print_info "You will now install the Affinity application using its Windows installer"
...
if ! ls "$directory"/*.exe 1> /dev/null 2>&1; then
    ...
    print_info "Please ensure the Affinity installer is in the correct location and rerun this script"
    exit 1
fi
...
print_step "Launching Affinity installer..."
...
affinity_dir="$directory/drive_c/Program Files/Affinity/Affinity"
wine_lib_dir="$directory/ElementalWarriorWine/lib/wine/vkd3d-proton/x86_64-windows"
...
if [ -d "$affinity_dir" ] && [ -d "$wine_lib_dir" ]; then
    ...
    if [ ! -d "$affinity_dir" ]; then
        print_warning "Affinity installation directory not found. OpenCL configuration skipped."
    fi
...

{
    ...
    echo "Icon=/home/$USER/.local/share/icons/Affinity.svg"
    echo "Path=$directory"
    echo "Exec=env WINEPREFIX=$directory ${dxvk_env}$directory/ElementalWarriorWine/bin/wine \"$directory/drive_c/Program Files/Affinity/Affinity/Affinity.exe\""
    echo "Terminal=false"
    echo "NoDisplay=false"
    echo "StartupWMClass=affinity.exe"
...
mkdir -p ~/Desktop
cp "$desktop_file" ~/Desktop/Affinity.desktop 2>/dev/null || true
...
print_success "Affinity has been successfully installed!"
echo ""
print_info "Installation location: $directory"
print_info "Desktop entry: ~/.local/share/applications/Affinity.desktop"
echo ""
print_info "You can now launch Affinity from:"
echo -e "  ${CYAN}•${NC} Your application menu (search for 'Affinity')"
echo -e "  ${CYAN}•${NC} The desktop shortcut"
echo -e "  ${CYAN}•${NC} Command line: wine $directory/drive_c/Program\ Files/Affinity/Affinity/Affinity.exe"
echo ""
...
echo ""
print_success "Enjoy using Affinity on Linux!"
